...
@section('headTitle',__('admin.jenispelayanans'))
@push('css')
...
<!-- Content Header (Page header) -->
    <section class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1>{{__('admin.jenispelayanans')}}</h1>
          </div>
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              <li class="breadcrumb-item"><a href="#">Home</a></li>
              <li class="breadcrumb-item active">{{__('admin.jenispelayanans')}}</li>
            </ol>
          </div>
        </div>
      </div><!-- /.container-fluid -->
            ...
            <div class="card">
              ...
              <div class="card p-2">
                <div class="card-header">
                  @can('add_jenispelayanans')
                  <div><a class="btn btn-primary" href="{{url('admin/jenispelayanans/create')}}">Add New</a></div>
                  @endcan
                  <div class="card-tools">
                    <div class="input-group input-group-sm">
                      {!! Form::open(['url' => route('jenispelayanans.index'), 'method' => 'GET']) !!}
                        <div class="row">
                          <div class="col">
                  ...
                  <table class="table table-hover table-fixed-head">
                    <thead>
                      <tr>
                        ...
                        <th>Created At</th>
                        <th>Updated At</th>
                        @can('edit_jenispelayanans', 'delete_jenispelayanans')
                        <th>Action</th>
                        @endcan
                      </tr>
                    </thead>
                    <tbody>
                      @forelse ($jenispelayanans as $jenispelayanan)
                        <tr>
                          <td>{{$jenispelayanan->id}}</td>
                          <td>{{$jenispelayanan->name}}</td>
                          <td>{{$jenispelayanan->created_at}}</td>
                          <td>{{$jenispelayanan->updated_at}}</td>
                          @can('edit_jenispelayanans', 'delete_jenispelayanans')
                          <td class="row w-100">
                          <a class="btn btn-primary btn-sm mr-1" href="{{url('admin/jenispelayanans/'.$jenispelayanan->id.'/edit')}}" role="button">
                            <i class="fa fa-fw fa-pen" aria-hidden="true"></i>
                            Edit</a>
                          {!! Form::open(['url' => 'admin/jenispelayanans/'.$jenispelayanan->id,'class'=> 'delete']) !!}
...
<script>
  ...
  $(function(){
    ...
    $('#daterange').daterangepicker({
      ...
      startDate: (startDate != null)?starDate:"{{date('Y-M-d')}}",
      endDate: endDate
    })
  })
